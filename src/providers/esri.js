...
export class ESRIWorld {
    ...
    constructor() {
        ...
        async function getData(suggestions) {
            const places = Promise.all(
                suggestions.map(
                    async place =>
                        await (await fetch(
                            `${this.settings.url.suggest}?singleLine="${place.text}"&magicKey="${
                            place.magicKey
                            }"&f=json`
                        )).json()
                )
    ...
    getParameters(opt) {
        return {
            url: this.settings.url.find,
            params: {
                text: opt.query,
                f: this.settings.params.f,
                maxSuggestions: opt.maxSuggestions || this.settings.params.maxSuggestions,
                countryCode: opt.countryCode || this.settings.params.countryCode,
            }
        };
    }
    ...
    handleResponse(results, callback) {
        if (results && results.suggestions && results.suggestions.length) {
            // getData(results.suggestions).then(data => {
